...
describe("buildIntelligenceOverview", () => {
  it("consolida somente métricas agregadas dos diretórios", () => {
    ...
    expect(overview.identityCoverage).toEqual({
      complete: 1,
      incomplete: 1,
      percentage: 50,
    });
    expect(overview.organizationLinks).toEqual({
      organizationsWithLinks: 1,
      explicitLinks: 2,
      linkedIndividuals: 1,
    });
    expect(overview.warrantAttention).toMatchObject({
      expiring: 1,
      expired: 1,
