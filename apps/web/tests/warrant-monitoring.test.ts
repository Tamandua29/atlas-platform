...
describe("classifyWarrantAttention", () => {
  it("prioriza estados finais informados pela fonte", () => {
    expect(classifyWarrantAttention("Cumprido", "2027-01-01", now)).toBe(
      "closed",
    );
  });

  it("sinaliza validade próxima sem declarar validade jurídica", () => {
    expect(classifyWarrantAttention("Vigente", "2026-08-25", now)).toBe(
      "expiring",
    );
  });

  it("sinaliza data vencida", () => {
    expect(classifyWarrantAttention("Ativo", "2026-08-01", now)).toBe(
      "expired",
    );
  });
