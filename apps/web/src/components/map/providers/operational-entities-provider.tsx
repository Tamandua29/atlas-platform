...
export const OperationalEntitiesContext =
  createContext<OperationalEntitiesContextValue | null>(null);
...
export function OperationalEntitiesProvider({
  ...
  const [state, setState] = useState<OperationalEntitiesState>(INITIAL_STATE);

  const applySuccessfulResult = useCallback(
  ...
  useEffect(() => {
    ...
    fetchOperationalEntities(controller.signal)
      .then((result) => {
  ...
  }, [applyError, applySuccessfulResult]);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const result = await fetchOperationalEntities();
  ...
  const contextValue = useMemo<OperationalEntitiesContextValue>(
    () => ({
      ...
      isEmpty: state.status === "success" && state.entities.length === 0,
  ...
  return (
    <OperationalEntitiesContext.Provider value={contextValue}>
      {children}
