...
export type DirectoryFilter = "all" | "judicial-attention" | "documents-missing" | "complete";
...
export function classifyDirectoryPriority(
  ...
  if (judicialAttention === "active" || judicialAttention === "expiring") return "high";
...
export function filterIndividualDirectory<T extends FilterableIndividual>(
  ...
  return individuals.filter((individual) => {
    const matchesSearch = !search || [individual.legalName, individual.alias || ""]
      .some((value) => normalized(value).includes(search));
    if (!matchesSearch) return false;

    if (filter === "judicial-attention") return individual.operationalPriority !== "normal";
