...
export async function GET(request: NextRequest) {
  ...
  const validation = validateUnifiedSearchQuery(
    request.nextUrl.searchParams.get("q") || "",
  );
  const rawCategory = request.nextUrl.searchParams.get("category") || "all";
  if (!validation.valid)
    return NextResponse.json(
      { success: false, correlationId, message: validation.message },
      { status: 400 },
    );
  if (!isUnifiedSearchCategory(rawCategory))
    return NextResponse.json(
      {
        success: false,
        correlationId,
        message: "Categoria de pesquisa inválida.",
      },
      { status: 400 },
    );
  ...
  try {
    ...
    const results = searchUnifiedIntelligence(
      { individuals, organizations, vehicles, warrants },
      validation.query,
      rawCategory,
    );
    const auditPersisted = await persistAuditSafely(
      AuditEntry.create({
        action: "intelligence.unified-search.query",
        outcome: "success",
        occurredAt: new Date(),
        correlationId,
    ...
    return NextResponse.json(
      {
        ...
        auditPersisted,
        correlationId,
        query: { length: validation.query.length, category: rawCategory },
    ...
    return NextResponse.json(
      {
        success: false,
        correlationId,
        message: "Não foi possível concluir a pesquisa protegida.",
      },
      { status: 500 },
    );
  }
