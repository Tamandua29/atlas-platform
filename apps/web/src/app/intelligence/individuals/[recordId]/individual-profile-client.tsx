    ...
    const timer = window.setTimeout(() => {
      void fetch(`/api/intelligence/individuals/${recordId}`, {
        ...
        .then(async (response) => {
          ...
          setPersonalRelationships(
            payload.relationships?.personalRelationships || [],
          );
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-7 px-6 py-10">
        ...
        <div className="flex flex-col justify-between gap-6 md:flex-row">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
            <div
              ...
              aria-label={
                individual.mainPhoto
                  ? `Foto principal de ${individual.legalName}`
                  : undefined
              }
              ...
              style={
                individual.mainPhoto
                  ? {
                      backgroundImage: `url("${individual.mainPhoto.thumbnailUrl}")`,
                    }
                  : undefined
              }
            >
      ...
      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Info
          label="Nascimento"
          value={individual.birthDate || "Não informado"}
        />
        <Info
          label="Filiação materna"
          value={individual.motherName || "Não informada"}
        />
        <Info
          label="CPF"
          value={individual.cpfPresent ? "Documento presente" : "Não informado"}
        />
        <Info
          label="RG"
          value={
            individual.identityDocumentPresent
              ? "Documento presente"
              : "Não informado"
          }
        />
      ...
      <section
        id="fotografias"
        className="scroll-mt-28 rounded-2xl border border-slate-800 bg-slate-900/60"
      >
          ...
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Acervo visual autorizado, filtrado por tipo de evidência e
                acesso auditado.
              </p>
            </div>
        ...
        <div className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {photos.length === 0 ? (
            <p className="text-slate-500">
              Nenhuma fotografia vinculada foi localizada.
            </p>
          ) : (
            photos.map((photo) => (
              <article
                key={`${photo.evidenceRecordId}-${photo.attachmentId}`}
        ...
        <p className="border-t border-slate-800 px-6 py-4 text-xs text-amber-300">
          Proteção ativa: somente anexos de imagem classificados como fotografia
          ou imagem de câmera são enviados.
        </p>
      </section>
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Locais vinculados à pessoa, sem alteração dos registros de
                origem.
              </p>
            </div>
            <span className="rounded-full bg-cyan-400/10 px-4 py-2 text-sm font-semibold text-cyan-300">
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2">
          {addresses.length === 0 ? (
            <p className="text-slate-500">
              Nenhum endereço vinculado foi localizado.
            </p>
          ) : (
                  ...
                  {mapUrl && mapEmbedUrl ? (
                    <div className="mt-5 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-3">
                        <a
                          href={`/operational-map?focusRecordId=${encodeURIComponent(recordId)}`}
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Linhas relacionadas à pessoa, exibidas de forma mascarada e
                auditada.
              </p>
            </div>
            <span className="rounded-full bg-cyan-400/10 px-4 py-2 text-sm font-semibold text-cyan-300">
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-3">
          {phones.length === 0 ? (
            <p className="text-slate-500">
              Nenhum telefone vinculado foi localizado.
            </p>
          ) : (
            phones.map((phone) => (
              <article
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Veículos relacionados à pessoa, com identificação protegida e
                acesso auditado.
              </p>
            </div>
            <span className="rounded-full bg-cyan-400/10 px-4 py-2 text-sm font-semibold text-cyan-300">
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-3">
          {vehicles.length === 0 ? (
            <p className="text-slate-500">
              Nenhum veículo vinculado foi localizado.
            </p>
          ) : (
            vehicles.map((vehicle) => (
              <article
                key={vehicle.recordId}
                className="rounded-2xl border border-slate-800 bg-slate-950/50 p-5"
              >
        ...
        <p className="border-t border-slate-800 px-6 py-4 text-xs text-amber-300">
          Proteção ativa: placa parcialmente mascarada e RENAVAM não enviado ao
          navegador.
        </p>
      </section>
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Referências judiciais vinculadas à pessoa, com dados sensíveis
                protegidos.
              </p>
            </div>
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2">
          {warrants.length === 0 ? (
            <p className="text-slate-500">
              Nenhum mandado vinculado foi localizado.
            </p>
          ) : (
            warrants.map((warrant) => (
              <article
                key={warrant.recordId}
                className="rounded-2xl border border-amber-400/20 bg-slate-950/50 p-5"
              >
        ...
        <p className="border-t border-slate-800 px-6 py-4 text-xs text-amber-300">
          Proteção ativa: referências mascaradas; detalhes, observações e
          documentos não são enviados ao navegador.
        </p>
      </section>
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Referências operacionais relacionadas à pessoa, com narrativa e
                anexos protegidos.
              </p>
            </div>
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2">
          {occurrences.length === 0 ? (
            <p className="text-slate-500">
              Nenhuma ocorrência vinculada foi localizada.
            </p>
          ) : (
            occurrences.map((occurrence) => (
              <article
                key={occurrence.recordId}
                className="rounded-2xl border border-rose-400/20 bg-slate-950/50 p-5"
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  ...
                  <OccurrenceInfo
                    ...
                    value={occurrence.confidence}
                  />
                  <OccurrenceInfo
                    label="Verificação"
                    value={occurrence.verificationStatus}
                  />
                </div>
              </article>
            ))
        ...
        <p className="border-t border-slate-800 px-6 py-4 text-xs text-amber-300">
          Proteção ativa: descrição, resultado operacional, observações e anexos
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Vínculos organizacionais registrados na fonte, sem inferência
                automática de pertencimento.
              </p>
            </div>
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-3">
          {organizations.length === 0 ? (
            <p className="text-slate-500">
              Nenhuma organização explicitamente vinculada foi localizada.
            </p>
          ) : (
            organizations.map((organization) => (
              <article
                key={organization.linkRecordId}
                className="rounded-2xl border border-violet-400/20 bg-slate-950/50 p-5"
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  <OrganizationInfo
                    label="Função ou posição"
                    value={organization.role}
                  />
                  <OrganizationInfo
                    label="Tipo de vínculo"
                    value={organization.relationshipType}
                  />
                  <OrganizationInfo
                    label="Situação da informação"
                    value={organization.informationStatus}
                  />
                  <OrganizationInfo
                    label="Confiabilidade"
                    value={organization.confidence}
                  />
                  <OrganizationInfo
                    label="Verificação"
        ...
        <p className="border-t border-slate-800 px-6 py-4 text-xs text-amber-300">
          Proteção ativa: somente vínculos explícitos e metadados mínimos são
          exibidos; observações e evidências não são enviadas ao navegador.
        </p>
      </section>
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Relações explicitamente registradas na fonte, sem inferência
                automática.
              </p>
            </div>
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-3">
          {personalRelationships.length === 0 ? (
            <p className="text-slate-500">
              Nenhum vínculo explícito entre pessoas foi localizado.
            </p>
          ) : (
            personalRelationships.map((relationship) => (
              <article
                key={relationship.relationshipRecordId}
                className="rounded-2xl border border-sky-400/20 bg-slate-950/50 p-5"
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  ...
                  <RelationshipInfo
                    label="Verificação"
                    value={relationship.verificationStatus}
                  />
                  <RelationshipInfo
                    label="Data da informação"
                    value={relationship.informationDate}
                  />
                  <RelationshipInfo
                    label="Categoria de risco"
                    value={relationship.riskCategory}
                  />
                  <RelationshipInfo
                    label="Classificação"
                    value={relationship.informationClassification}
                  />
                  <RelationshipInfo
                    label="Fonte"
                    value={
                      relationship.sourceRegistered
        ...
        <p className="border-t border-slate-800 px-6 py-4 text-xs text-amber-300">
          Proteção ativa: descrições, observações, resumos de IA e identificação
          da fonte não são enviados ao navegador. O vínculo não confirma
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              <h3 className="text-2xl font-semibold text-white">
                Relatórios e documentos vinculados
              </h3>
              <p className="mt-2 text-slate-400">
                Documentos explicitamente ligados à pessoa, com conteúdo e
                anexos protegidos.
              </p>
            </div>
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-3">
          {documents.length === 0 ? (
            <p className="text-slate-500">
              Nenhum relatório ou documento vinculado foi localizado.
            </p>
          ) : (
            documents.map((document) => (
              <article
                key={document.recordId}
                className="rounded-2xl border border-teal-400/20 bg-slate-950/50 p-5"
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  ...
                  <DocumentInfo
                    ...
                    value={document.originAgency}
                  />
                  <DocumentInfo
                    label="Classificação"
                    value={document.informationClassification}
                  />
                  <DocumentInfo
                    label="Arquivos protegidos"
                    value={document.protectedAttachmentCount.toString()}
                  />
                  <DocumentInfo
                    label="Ocorrências relacionadas"
                    value={document.occurrenceReferenceCount.toString()}
                  />
                  <DocumentInfo
                    label="Evidências relacionadas"
                    value={document.evidenceReferenceCount.toString()}
                  />
                </div>
              </article>
            ))
        ...
        <p className="border-t border-slate-800 px-6 py-4 text-xs text-amber-300">
          Proteção ativa: arquivos, URLs, observações, conteúdo, responsável e
          identificação detalhada da fonte não são enviados ao navegador.
        </p>
      </section>
      ...
      >
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-400">
                Eventos datados e ordenados cronologicamente a partir dos
        ...
        <div className="p-6">
          {timeline.length === 0 ? (
            <p className="text-slate-500">
              Nenhum evento com data válida foi localizado.
            </p>
          ) : (
        ...
        <p className="border-t border-slate-800 px-6 py-4 text-xs text-amber-300">
          Proteção ativa: somente eventos com data válida e metadados mínimos
          são exibidos; narrativas, anexos e dados documentais integrais
