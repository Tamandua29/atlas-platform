...
export function VehicleDirectoryClient() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch("/api/intelligence/vehicles?limit=100", {
        cache: "no-store",
        ...
        .then(async (response) => {
          ...
          if (!response.ok || !payload.success)
            throw new Error(payload.message || "Falha ao consultar veículos.");
  ...
  const filtered = useMemo(
    () => filterVehicleDirectory(vehicles, query),
    [query, vehicles],
  );
    ...
    <div className="mx-auto max-w-[1500px] space-y-7 px-6 py-10">
      <section className="rounded-3xl border border-cyan-400/20 bg-gradient-to-r from-slate-900 to-cyan-950/40 p-8">
        <p className="text-sm font-bold uppercase tracking-[0.2em] text-cyan-400">
          Consulta operacional protegida
        </p>
        <h2 className="mt-4 text-4xl font-bold text-white">
          Diretório seguro de veículos
        </h2>
        <p className="mt-4 max-w-4xl text-slate-400">
          Veículos cadastrados na fonte com placa parcialmente mascarada. O
          RENAVAM e outros identificadores integrais não são enviados ao
          navegador.
        </p>
      </section>
        ...
        {filtered.map((vehicle) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="rounded-full border border-amber-400/30 bg-amber-950/30 px-3 py-1 text-sm font-semibold text-amber-200">
                {vehicle.maskedPlate}
              </span>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
              ...
              <span className="rounded-lg bg-slate-950/70 p-3">
                Situação: {vehicle.status || "não informada"}
              </span>
              <span className="rounded-lg bg-slate-950/70 p-3">
                Vínculo: {vehicle.relationshipType || "não informado"}
              </span>
            </div>
            <Link
      ...
      {!status && filtered.length === 0 ? (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-8 text-center text-slate-400">
          Nenhum veículo corresponde à consulta.
        </div>
      ) : null}
    </div>
  );
