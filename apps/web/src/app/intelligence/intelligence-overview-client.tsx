...
function loadOverview() {
  overviewRequest ??= fetch("/api/intelligence/overview", {
    cache: "no-store",
  }).then(async (response) => {
    const payload = (await response.json()) as ResponsePayload;
    if (!response.ok || !payload.success || !payload.overview) {
      throw new Error(
        payload.message || "Não foi possível carregar a visão consolidada.",
      );
    }
    return payload;
  });
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Somente indicadores agregados, sem dados pessoais.
          </p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
        <article className={metricClass}>
          ...
          <p className="mt-3 text-3xl font-bold text-cyan-300">
            {overview.identityCoverage.percentage}%
          </p>
          <div
            className="mt-3 h-2 overflow-hidden rounded-full bg-slate-800"
            role="progressbar"
            aria-valuenow={overview.identityCoverage.percentage}
            aria-valuemin={0}
            aria-valuemax={100}
          >
            <div
              className="h-full rounded-full bg-cyan-400"
              style={{ width: `${overview.identityCoverage.percentage}%` }}
            />
          </div>
          <p className="mt-3 text-sm text-slate-400">
            {overview.identityCoverage.complete} completas ·{" "}
            {overview.identityCoverage.incomplete} incompletas
          </p>
        </article>
        <article className={metricClass}>
          <p className="font-semibold text-white">Vínculos organizacionais</p>
          <p className="mt-3 text-3xl font-bold text-violet-300">
            {overview.organizationLinks.explicitLinks}
          </p>
          <p className="mt-3 text-sm text-slate-400">
            {overview.organizationLinks.organizationsWithLinks} organizações com
            vínculo · {overview.organizationLinks.linkedIndividuals} referências
            individuais
          </p>
        </article>
        <article className={metricClass}>
          <p className="font-semibold text-white">
            Mandados que exigem atenção
          </p>
          <p className="mt-3 text-3xl font-bold text-amber-300">
            {overview.warrantAttention.requiringAttention}
          </p>
          <p className="mt-3 text-sm text-slate-400">
            {overview.warrantAttention.active} ativos ·{" "}
            {overview.warrantAttention.expiring} próximos do vencimento ·{" "}
            {overview.warrantAttention.expired} vencidos
          </p>
        </article>
        <article className={metricClass}>
          <p className="font-semibold text-white">Situação dos veículos</p>
          <p className="mt-3 text-3xl font-bold text-sky-300">
            {overview.vehicleStatus.informed}
          </p>
          <p className="mt-3 text-sm text-slate-400">
            com situação informada · {overview.vehicleStatus.notInformed}{" "}
            pendentes
          </p>
        </article>
      </div>
