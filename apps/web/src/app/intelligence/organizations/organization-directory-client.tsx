    ...
    const timer = window.setTimeout(() => {
      void fetch("/api/intelligence/organizations?limit=100", {
        ...
        .then(async (response) => {
          ...
          if (!response.ok || !payload.success) throw new Error(payload.message || "Falha ao consultar organizações.");
        ...
        .catch((error: unknown) => {
          if (!controller.signal.aborted) {
            setStatus(error instanceof Error ? error.message : "Falha ao consultar organizações.");
          }
        });
    }, 0);
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-7 px-6 py-10">
      <section className="rounded-3xl border border-cyan-400/20 bg-gradient-to-r from-slate-900 to-cyan-950/40 p-8">
        <p className="text-sm font-bold uppercase tracking-[0.2em] text-cyan-400">Vínculos expressamente registrados</p>
        <h2 className="mt-4 text-4xl font-bold text-white">Diretório seguro de organizações</h2>
        <p className="mt-4 max-w-4xl text-slate-400">
          Consulta autenticada e auditada de organizações cadastradas na fonte. A presença nesta lista ou um vínculo registrado não confirma participação criminal.
        </p>
      </section>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="text-sm uppercase tracking-[0.16em] text-slate-500">Organizações localizadas</p>
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((organization) => (
          <article key={organization.recordId} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
            <p className="text-xs uppercase tracking-[0.16em] text-violet-300">Organização cadastrada</p>
            <h3 className="mt-3 text-xl font-semibold text-white">{organization.name}</h3>
            <p className="mt-1 text-slate-400">{organization.acronym || "Sem sigla informada"}</p>
            <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
              <span className="rounded-lg bg-slate-950/70 p-3">Tipo: {organization.organizationType || "não informado"}</span>
              <span className="rounded-lg bg-slate-950/70 p-3">Situação: {organization.organizationStatus || "não informada"}</span>
            </div>
