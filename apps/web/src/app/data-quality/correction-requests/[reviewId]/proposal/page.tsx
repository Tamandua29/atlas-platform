...
import { CorrectionProposalClient } from "./correction-proposal-client";

type PageProps = {
  params: Promise<{ reviewId: string }>;
};

export default async function CorrectionProposalPage({ params }: PageProps) {
        ...
        <div className="mx-auto flex min-h-20 max-w-[1500px] items-center justify-between gap-4 px-5 py-4 md:px-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="mt-1 text-lg font-semibold text-white">Proposta protegida de correção</h1>
            </div>
          </div>
          <Link href="/data-quality/correction-requests" className="rounded-xl border border-slate-700 px-4 py-2.5 text-sm text-slate-300">Voltar às solicitações</Link>
        </div>
      </header>
      <CorrectionProposalClient reviewId={reviewId} />
    </main>
  );
}
