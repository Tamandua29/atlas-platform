...
export function CorrectionProposalClient({ reviewId }: { reviewId: string }) {
  ...
  async function load() {
    ...
    try {
      const [proposalResponse, sessionResponse] = await Promise.all([
        fetch(`/api/data-quality/correction-requests/${reviewId}/proposal`, {
          cache: "no-store",
        }),
      ...
      if (!proposalResponse.ok || !payload.success || !payload.proposal) {
        throw new Error(
          payload.message ?? "Não foi possível carregar a proposta.",
        );
      }
  ...
  async function save() {
    ...
    try {
      ...
      if (!response.ok || !payload.success || !payload.proposal) {
        throw new Error(
          payload.message ?? "Não foi possível salvar a proposta.",
        );
      }
      setContext(payload.proposal);
      setMessage(
        "Proposta registrada e encaminhada para aprovação humana. Nenhum cadastro de origem foi alterado.",
      );
      setValues({});
    } catch (caught) {
      setError(
  ...
  async function decide(decision: "approve" | "reject") {
    if (decisionReason.trim().length < 10) {
      setError(
        "A justificativa da decisão deve possuir pelo menos 10 caracteres.",
      );
      return;
    }
  ...
  async function executeCorrection() {
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(
          payload.message ?? "Não foi possível executar a correção.",
        );
      }
      setExecutionNote("");
      setConfirmation("");
  ...
  async function submitReversal(
    ...
    const reason =
      action === "request" ? reversalReason : reversalDecisionReason;
    if (action !== "execute" && reason.trim().length < 10) {
      setError(
        "A fundamentação da reversão deve possuir pelo menos 10 caracteres.",
      );
      return;
    }
    if (action === "execute") {
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(
          payload.message ?? "Não foi possível concluir a etapa de reversão.",
        );
      }
      setReversalReason("");
    ...
    <div className="mx-auto max-w-[1500px] px-5 py-6 md:px-8 md:py-8">
      <section className="rounded-3xl border border-cyan-400/15 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.13),transparent_32%),linear-gradient(135deg,#0b1326,#080d1b)] p-6 md:p-8">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-cyan-400">
          Dupla barreira de segurança
        </p>
        <h2 className="mt-3 text-3xl font-semibold text-white md:text-4xl">
          Antes × valor proposto
        </h2>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
          A proposta fica separada do cadastro e vinculada à versão consultada.
          Salvar não aplica nenhuma alteração.
        </p>
        <div className="mt-5 flex flex-wrap gap-3 text-xs">
          ...
          <span className="rounded-full border border-amber-400/20 bg-amber-400/10 px-3 py-2 text-amber-200">
            {context?.executionStatus === "Aplicada"
              ? "1 escrita controlada na origem"
              : "0 escritas na origem"}
          </span>
      ...
      {loading ? (
        <div className="mt-6 rounded-2xl border border-slate-800 bg-[#0a1020] p-10 text-center text-slate-500">
          Carregando contexto protegido...
        </div>
      ) : (
        context && (
          <section className="mt-6 overflow-hidden rounded-2xl border border-slate-800 bg-[#0a1020]">
            <div className="border-b border-slate-800 p-5">
              <h3 className="font-semibold text-white">
                Campos autorizados pela pendência
              </h3>
            ...
            <div className="space-y-4 p-5">
              {context.allowedFields.map((field) => (
                <div
                  key={field}
                  className="grid gap-4 rounded-2xl border border-slate-800 bg-slate-950/45 p-5 md:grid-cols-2"
                >
                  <div>
                    ...
                    <p className="mt-3 text-sm text-slate-300">
                      {context.current[field] ?? "Não informado"}
                    </p>
                  </div>
                  <div>
                    ...
                    {hasProposal ? (
                      <p className="mt-3 text-sm text-cyan-100">
                        {context.proposed?.[field] ?? "Sem alteração proposta"}
                      </p>
                    ) : (
                      <input
            ...
            {hasProposal &&
              context.proposalStatus === "Pendente de aprovação" && (
                <div className="border-t border-slate-800 p-5">
                  {actorId === context.proposerId ? (
                    <div className="rounded-xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm text-amber-200">
                      Segregação de funções ativa: a identidade que elaborou
                      esta proposta não pode decidi-la. Autentique uma segunda
                      identidade revisora.
                    </div>
                  ) : canWrite ? (
                    ...
                    <div className="rounded-xl border border-slate-700 bg-slate-950/45 p-4 text-sm text-slate-400">
                      Perfil somente leitura: a decisão exige uma identidade
                      revisora diferente da proponente.
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="border-t border-slate-800 p-5">
                  <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-4 text-sm text-emerald-100">
                    <p className="font-semibold">
                      Decisão: {context.proposalStatus}
                    </p>
                    <p className="mt-2 text-xs text-emerald-100/70">
                      Decisor: {context.approverId ?? "Não identificado"}
                    </p>
                    ...
                    {context.proposalStatus === "Aprovada" && (
                      <p className="mt-2 text-xs text-amber-200/80">
                        A aprovação ainda não aplicou nenhuma alteração ao
                        cadastro.
                      </p>
                    )}
                  </div>
            ...
            {context.proposalStatus === "Aprovada" &&
              !context.executionStatus && (
                <div className="border-t border-slate-800 p-5">
                  {actorId === context.proposerId ||
                  actorId === context.approverId ? (
                    <div className="rounded-xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm text-amber-200">
                      Terceira identidade obrigatória: proponente e aprovador
                      não podem executar a escrita controlada.
                    </div>
                  ) : canWrite ? (
                    <div className="rounded-2xl border border-rose-400/20 bg-slate-950/45 p-5">
                      <p className="text-xs font-semibold uppercase tracking-wider text-rose-300">
                        Execução controlada na origem
                      </p>
                      <p className="mt-2 text-sm leading-6 text-slate-400">
                        O hash da versão será conferido novamente. Somente os
                        campos aprovados serão atualizados e o estado anterior
                        será preservado.
                      </p>
                      <textarea
                        rows={3}
                      ...
                      />
                      <label
                        htmlFor="execution-confirmation"
                      ...
                      <div className="mt-4 flex justify-end">
                        ...
                        >
                          {executing
                            ...
                            : "Executar correção aprovada"}
                        </button>
                      </div>
                    </div>
                  ) : (
                    <div className="rounded-xl border border-slate-700 bg-slate-950/45 p-4 text-sm text-slate-400">
                      A execução exige uma terceira identidade com perfil
                      Revisor.
                    </div>
                  )}
                </div>
            ...
            {(context.executionStatus === "Aplicada" ||
              context.proposalStatus === "Aplicada") && (
              <div className="border-t border-slate-800 p-5">
                <div className="rounded-xl border border-emerald-400/25 bg-emerald-400/10 p-4 text-sm text-emerald-100">
                  <p className="font-semibold">
                    Correção aplicada com controle de versão
                  </p>
                  <p className="mt-2 text-xs">
                    Executor: {context.executorId ?? "Não identificado"}
                  </p>
                  <p className="mt-2 text-xs">
                    Execução: {context.executedAt ?? "Data não informada"}
                  </p>
                  <p className="mt-2 text-xs text-emerald-100/70">
                    Snapshot anterior e hashes preservados para auditoria e
                    recuperação.
                  </p>
                </div>
              </div>
            ...
            {(context.executionStatus === "Aplicada" ||
              ...
              <div className="border-t border-slate-800 p-5">
                <div className="rounded-2xl border border-amber-400/20 bg-slate-950/45 p-5">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-400">
                    A restauração usa somente o snapshot preservado e é
                    bloqueada se o cadastro tiver sofrido qualquer alteração
                    posterior.
                  </p>
                  ...
                  {!context.reversalStatus &&
                    (actorId === context.executorId ? (
                      <div className="mt-4 rounded-xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm text-amber-200">
                        O executor original não pode solicitar a reversão da
                        própria escrita. Autentique uma identidade revisora
                        segregada.
                      </div>
                    ) : canWrite ? (
                      <div className="mt-4">
                      ...
                      <p className="mt-4 text-sm text-slate-500">
                        A solicitação exige perfil Revisor.
                      </p>
                    ))}
                  ...
                  {context.reversalStatus === "Pendente de aprovação" && (
                    <div className="mt-4">
                      <div className="rounded-xl border border-slate-700 p-4 text-sm text-slate-300">
                        <p className="font-semibold">
                          Reversão pendente de aprovação
                        </p>
                        <p className="mt-2 text-xs">
                          Solicitante:{" "}
                          {context.reversalRequesterId ?? "Não identificado"}
                        </p>
                        <p className="mt-2">{context.reversalReason}</p>
                      </div>
                      {actorId === context.reversalRequesterId ||
                      actorId === context.executorId ? (
                        <div className="mt-4 rounded-xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm text-amber-200">
                          A decisão exige identidade distinta do solicitante e
                          do executor original.
                        </div>
                      ) : (
                        canWrite && (
                          <>
                            <textarea
                              ...
                              onChange={(event) =>
                                setReversalDecisionReason(event.target.value)
                              }
                            ...
                            <div className="mt-4 flex flex-wrap justify-end gap-3">
                              <button
                                ...
                                disabled={reversing}
                                onClick={() =>
                                  void submitReversal("decide", "reject")
                                }
                                className="rounded-xl border border-rose-400/30 bg-rose-400/10 px-5 py-3 text-sm font-semibold text-rose-200 disabled:opacity-60"
                              >
                                Rejeitar reversão
                              ...
                              <button
                                ...
                                disabled={reversing}
                                onClick={() =>
                                  void submitReversal("decide", "approve")
                                }
                    ...
                    (actorId === context.reversalApproverId ? (
                      <div className="mt-4">
                        <p className="text-sm text-amber-100">
                          Reversão aprovada. O hash atual será conferido antes
                          de qualquer escrita.
                        </p>
                        <textarea
                          rows={3}
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            setReversalConfirmation(event.target.value)
                          }
                          autoComplete="off"
                          className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white outline-none focus:border-rose-400"
                        />
                      ...
                      <div className="mt-4 rounded-xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm text-amber-200">
                        A execução cabe exclusivamente à identidade segregada
                        que aprovou esta reversão.
                      </div>
                    ))}
                  ...
                  {context.reversalStatus === "Revertida" && (
                    <div className="mt-4 rounded-xl border border-cyan-400/25 bg-cyan-400/10 p-4 text-sm text-cyan-100">
                      <p className="font-semibold">
                        Snapshot anterior restaurado e reconciliado
                      </p>
                      <p className="mt-2 text-xs">
                        Executor:{" "}
                        {context.reversalExecutorId ?? "Não identificado"}
                      </p>
                      <p className="mt-2 text-xs">
                        Reversão: {context.revertedAt ?? "Data não informada"}
                      </p>
                    </div>
                  )}
                </div>
