  ...
  async function load(nextStatus: Status) {
    ...
    try {
      const [queueResponse, sessionResponse] = await Promise.all([
        fetch(`/api/data-quality/correction-requests?status=${nextStatus}`, {
          cache: "no-store",
        }),
      ...
      if (!queueResponse.ok || !payload.success) {
        throw new Error(
          payload.message ?? "Não foi possível consultar as solicitações.",
        );
      }
    ...
    try {
      const response = await fetch(
        `/api/data-quality/correction-requests/${item.reviewId}/transition`,
        {
          ...
          body: JSON.stringify({
            action,
            note: action === "complete" ? note : undefined,
          }),
        },
      );
      ...
      if (!response.ok || !payload.success) {
        throw new Error(
          payload.message ?? "Não foi possível atualizar a solicitação.",
        );
      }
      setMessage(
        action === "claim"
        ...
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-cyan-400">
              Fluxo humano controlado
            </p>
            <h2 className="mt-3 text-3xl font-semibold text-white md:text-4xl">
              Solicitações de correção cadastral
            </h2>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
              Assuma e conclua análises de saneamento com rastreabilidade. Esta
              etapa altera somente o fluxo da solicitação, nunca o cadastro da
              pessoa.
            </p>
          </div>
          <div className="rounded-2xl border border-emerald-400/20 bg-slate-950/45 px-5 py-4">
            ...
            <p className="mt-1 text-xs text-slate-500">
              {actorId ?? "Sessão não identificada"} · 0 escritas na origem
            </p>
          </div>
        </div>
      </section>
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h4 className="mt-2 font-mono text-sm text-white">
                      {item.reviewId}
                    </h4>
                  </div>
                  <span
                    className={`rounded-full border px-3 py-1 text-[10px] font-semibold uppercase ${statusClasses(item.status)}`}
                  >
                ...
                <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
                  <div>
                    ...
                    <dd className="mt-1 text-slate-300">
                      {item.openedOn ?? "Não informada"}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-slate-600">Responsável</dt>
                    <dd className="mt-1 text-slate-300">
                      {item.assigneeId ?? "Não atribuída"}
                    </dd>
                  </div>
                </dl>
