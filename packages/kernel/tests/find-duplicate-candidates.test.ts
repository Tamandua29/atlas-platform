...
function individual(
  ...
  return CanonicalIndividual.create({
    legalName: overrides.legalName ?? "José da Silva",
    cpf: overrides.cpf,
    birthDate: overrides.birthDate,
    motherName: overrides.motherName,
    source: SourceRecordReference.create({
      system: "airtable",
      baseId: "app",
      tableId: "tbl",
      recordId,
...
describe("findDuplicateCandidates", () => {
  ...
  it("não deve sugerir duplicidade com dados insuficientes", () => {
    expect(
      findDuplicateCandidates([individual("rec1"), individual("rec2")]),
    ).toEqual([]);
  });
  ...
  it("deve preservar todas as origens para revisão", () => {
    ...
    expect(candidates[0]?.sourceKeys).toHaveLength(2);
  });
});
